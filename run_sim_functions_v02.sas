/************************************************************************************************************
PROJECT:	Paper 2
PROGRAM:	run_sim_functions_v02.sas
PROGRAMMER:     kat
DATE:		2/7/2017	
...
HISTORY:    	03.08.2017 Updating to look at interaction between A and D (a site-specific characteristic/D does not vary within site)
		05.07.2017 v02 will run on 2,000 simulations; not sure can run all together ... 
************************************************************************************************************/
...
%macro run_all(y=,x=);
	...
	*%mixed_pois_laplace();
	...
	/* 10b. COPY method with random intercept (method=Laplace) */
...
%macro setup(var2=, clustnum=,letter=); 
	libname in "&dir.";
	
	%if %upcase(&var2)=X2 and %upcase(&letter)= %then %do;
		 libname out "&dir./results_datasets/&clustnum.clust";
		
		 data simdata0;
			set in.sim_dat_&clustnum.clust_v03;
		 run;
	%end;
	%else %if %upcase(&var2)=X2 %then %do;
		 libname out "&dir./results_datasets/&clustnum.clust_&letter.";

		 data simdata0;
			set in.sim_dat_&clustnum.clust_&letter._v03;
		 run;	
	%end;
	...
	data simdata (keep=sim cluster id outcome outcomevalue x1 &var2.);
		set simdata0;
		by Sim Cluster ID;
	
		%if %upcase(&var2)=X2 %then %do;
			array outc (13) /*o_A1 o_A2 o_A3*/ o_A4 o_A5 o_B1 o_B2 o_B3 o_B4 o_C1 o_C2 o_C3 o_D1 o_D2 o_D3 o_E1;
		%end;
	...
	%run_all(y=outcomevalue, x=X1 X2 X1*&var2.);

%mend setup;

ods exclude all;

*%setup(var2=X2, clustnum=20, letter=);
*%setup(var2=X2, clustnum=20, letter=b);
%setup(var2=X2, clustnum=50, letter=);
*%setup(var2=X2, clustnum=50, letter=b);
%setup(var2=X2, clustnum=275, letter=a);
*%setup(var2=X2, clustnum=275, letter=b);
*%setup(var2=X2, clustnum=275, letter=c);
